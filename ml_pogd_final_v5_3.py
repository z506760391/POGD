"""
POGD Prediction Model v5.3 — Fully Patched Version (v5.1 + Review Fixes)
New Fixes (vs. v5.1):
  🔴 Fix 1. Feature drift detection data leakage: Use internal 80/20 train split for feature
            removal decisions; external set is used ONLY for baseline comparability reporting.
  🟠 Fix 2. Ensemble sensitivity analysis + bootstrap statistical comparison (Part 6B):
            Tests Top-N ∈ {2,3,5} with weighted and equal-weight strategies; bootstrap
            paired AUC-ROC comparison (1 000 resamples) replaces broken hardcoded-variance DeLong.
  🟠 Fix 3. Nested cross-validation (Part 4B): Outer 5-fold / inner 3-fold nested CV for the
            best-CV model to detect optimism in reported CV scores; best_s moved earlier.
  🟢 Fix 4. Code reproducibility: DATA_PATH changed to relative path; runtime generation of
            requirements.txt (pinned versions) and example_data_format.csv skeleton.
Fix List (vs. v4.6, carried over from v5.1):
  🔴 P1. Platt calibration leakage fix: Fit on the training set.
  🔴 P2. Ensemble weight leakage fix: Use inner-fold CV scores from GridSearchCV.
  🔴 P3. SHAP waterfall plot error fix: Unified handling of all output formats.
  🟠 P4. Dirty data cleaning: Use regular expressions to remove abnormal characters.
  🟠 P5. Feature distribution shift detection: KS test, auto-removal of high-drift features.
  ...
  🟢 P9. Feature distribution comparison plot for internal vs. external sets (TRIPOD requirement).
"""

# ============================================================
# [TOP PRIORITY] Force non-interactive backend to completely eliminate Tkinter multi-threading errors.
# This must be executed before importing matplotlib.pyplot!
# ============================================================
import matplotlib
matplotlib.use('Agg')   # ✅ Non-interactive backend, no Tkinter required, thread-safe.

# ============================================================
# [0] Other Imports and Global Configurations
# ============================================================
...
# Fix: Suppress a specific UserWarning from scikit-learn's parallel processing for a cleaner log.
# This warning is benign and does not affect the results, but it clutters the log output. UserWarning is a built-in type.
warnings.filterwarnings("ignore", category=UserWarning, module='sklearn.utils.parallel')

# ============================================================
# Chinese Font Configuration (Can still render Chinese with Agg backend)
# ============================================================
...
BOOTSTRAP_N        = 1000
CI_LEVEL           = 0.95
DATA_PATH          = 'data/20260411.csv'   # relative path; place your CSV in a 'data/' subdirectory
OUTPUT_DIR         = 'output'
TARGET_COL         = 'POGD'
...
def _step(msg):
    """Unified progress prompt"""
    print(f"\n  ▶ {msg}")

print("=" * 120)
print("POGD Clinical Prediction Model v5.3 (Fully Patched)".center(120))
print("=" * 120)
print(f"  Current font: {_active_font}")
print(f"  matplotlib backend: {matplotlib.get_backend()}  ✅ Agg forced, Tkinter multi-threading error eliminated")
...
drift_remove_feats = []
if has_ext:
    _step("训练集内部拆分做漂移检测（用于特征剔除，防止外部集泄露）...")
    Xtr_inner, Xtr_val_inner, _, _ = train_test_split(
        Xtr, ytr, test_size=0.2, random_state=RANDOM_SEED, stratify=ytr)
    _, drift_remove_feats = distribution_shift_report(Xtr_inner, Xtr_val_inner, feat_cols)
    if drift_remove_feats:
        print(f"  ⚠️  将剔除高漂移特征（KS>{KS_DRIFT_THRESHOLD}）: {drift_remove_feats}")
    else:
        print("  ✅ 未发现高漂移特征")
    # External set used ONLY for baseline comparability report — does NOT affect feature selection
    _step("外部集漂移检测（仅用于基线可比性报告，不影响特征剔除）...")
    distribution_shift_report(Xtr, Xe, feat_cols)
...
# ============================================================
# [Part 2] EasyEnsemble for Imbalance Handling
# ============================================================
print("\n" + "=" * 120)
...
print(f"  ✅ Done! Minority class: {len(Xmin)}, Majority class: {len(Xmaj)}, Size of each subset: {len(Xmin)*2}")

# ============================================================
# [Part 3] 10 Heterogeneous Base Classifiers (P6 Enhanced Regularization)
# ============================================================
print("\n" + "=" * 120)
print("[Part 3] Definition of 10 Heterogeneous Base Classifiers (P6: Enhanced Regularization to Prevent Overfitting)")
print("=" * 120)
...
for idx_m, (nm, cfg) in enumerate(clf_cfg.items()):
    ...
    try:
        ...
        print(f"Best params={gs.best_params_}  CV_AP={gs.best_score_:.4f}  ✅")
...
print(f"  CV最佳单模型: {best_s}  CV-AP={cv_scores[best_s]:.4f}")

# ============================================================
# 【第4B部分】嵌套交叉验证（验证泛化稳定性）
# ============================================================
print("\n" + "=" * 120)
print("【第4B部分】嵌套交叉验证（仅对CV最佳模型，验证泛化稳定性）")
print("=" * 120)

_step(f"对 [{best_s}] 进行嵌套CV (外层5折, 内层3折, AUC-PR)...")
from sklearn.model_selection import cross_val_score as _cvs
outer_cv_ncv = StratifiedKFold(n_splits=5, shuffle=True, random_state=RANDOM_SEED)
inner_cv_ncv = StratifiedKFold(n_splits=3, shuffle=True, random_state=RANDOM_SEED)
try:
    gs_ncv = GridSearchCV(
        clone(clf_cfg[best_s]['model']), clf_cfg[best_s]['params'],
        cv=inner_cv_ncv, scoring='average_precision', n_jobs=-1, error_score=0.)
    nested_scores = _cvs(gs_ncv, Xtr_sel, ytr,
                         cv=outer_cv_ncv, scoring='average_precision', n_jobs=-1)
    print(f"  嵌套CV AUC-PR: {nested_scores.mean():.4f} ± {nested_scores.std():.4f}")
    print(f"  95%CI (百分位): [{np.percentile(nested_scores,2.5):.4f}, {np.percentile(nested_scores,97.5):.4f}]")
    print(f"  各折: {[round(s,4) for s in nested_scores]}")
    print(f"  简单CV AUC-PR: {cv_scores[best_s]:.4f}  (乐观偏差={cv_scores[best_s]-nested_scores.mean():+.4f})")
    nested_cv_df = pd.DataFrame({'Fold': range(1,len(nested_scores)+1), 'AUC-PR': nested_scores})
    nested_cv_df.to_csv(out(f'nested_cv_{best_s}.csv'), index=False, encoding='utf-8-sig')
    print(f"  ✅ 嵌套CV结果已保存: {out(f'nested_cv_{best_s}.csv')}")
except Exception as e_ncv:
    print(f"  ⚠️ 嵌套CV失败: {e_ncv}")
...
# ============================================================
# 【第6B部分】集成策略敏感性分析 + Bootstrap统计检验
# ============================================================
print("\n" + "=" * 120)
print("【第6B部分】集成策略敏感性分析 + Bootstrap统计检验")
print("=" * 120)

_step("不同Top-N与权重策略的敏感性分析（内部验证集）...")
sens_rows = []
for n in [2, 3, 5]:
    if n > len(clf_names): continue
    top_n_list = [nm for nm, _ in sorted(cv_scores.items(),
                  key=lambda x: x[1] if not np.isnan(x[1]) else -1,
                  reverse=True)[:n]]
    # Weighted
    pw = wsv(bag_mods, Xva_sel, top_n_list, cv_scores)
    # Equal weight
    pe = wsv(bag_mods, Xva_sel, top_n_list, {nm: 1.0 for nm in top_n_list})
    for label, pp in [('Weighted', pw), ('Equal', pe)]:
        auc_roc = roc_auc_score(yn['va'], pp)
        auc_pr  = average_precision_score(yn['va'], pp)
        sens_rows.append({'Top-N': n, 'Strategy': label, 'AUC-ROC': round(auc_roc,4), 'AUC-PR': round(auc_pr,4)})
        print(f"  Top{n} {label:8s}  AUC-ROC={auc_roc:.4f}  AUC-PR={auc_pr:.4f}")
sens_df = pd.DataFrame(sens_rows)
sens_df.to_csv(out('ensemble_sensitivity_analysis.csv'), index=False, encoding='utf-8-sig')
print(f"  ✅ 敏感性分析结果已保存: {out('ensemble_sensitivity_analysis.csv')}")

_step("Bootstrap统计检验：集成模型 vs 最佳单模型（AUC-ROC，1000次重采样）...")
yt_cmp  = yn['va']
yp_ens  = ep['va']
yp_best = prob['va'][best_s]
rng_cmp = np.random.RandomState(RANDOM_SEED)
diff_boot = []
for _ in range(1000):
    idx_b = resample(np.arange(len(yt_cmp)), random_state=rng_cmp.randint(0, 100000))
    if len(np.unique(yt_cmp[idx_b])) < 2: continue
    d = roc_auc_score(yt_cmp[idx_b], yp_ens[idx_b]) - \
        roc_auc_score(yt_cmp[idx_b], yp_best[idx_b])
    diff_boot.append(d)
diff_boot = np.array(diff_boot)
obs_diff  = roc_auc_score(yt_cmp, yp_ens) - roc_auc_score(yt_cmp, yp_best)
p_boot    = 2 * min(np.mean(diff_boot <= 0), np.mean(diff_boot >= 0))
ci_lo_d   = float(np.percentile(diff_boot, 2.5))
ci_hi_d   = float(np.percentile(diff_boot, 97.5))
print(f"  集成 AUC-ROC={roc_auc_score(yt_cmp, yp_ens):.4f}  最佳单模型({best_s}) AUC-ROC={roc_auc_score(yt_cmp, yp_best):.4f}")
print(f"  差值={obs_diff:+.4f}  95%CI=[{ci_lo_d:+.4f}, {ci_hi_d:+.4f}]  Bootstrap p={p_boot:.4f}")
...
def run_shap_analysis(model, model_name, X_train_data, X_shap_data, y_shap_data, feature_names, is_ensemble=False):
    ...
    try:
        ...
        # 寻找中位风险个体 (最接近预测概率中位数的)
        median_prob = np.median(yp_sh)
        me_idx = int(np.argmin(np.abs(yp_sh - median_prob)))
        ...
        for risk_type, idx in [('high_risk', hi_idx), ('medium_risk', me_idx), ('low_risk', lo_idx)]:
            ...
            # 准备格式化后用于绘图的特征数据
            patient_features_rounded = X_shap_data.iloc[idx,:].round(1)
        ...
        try:
            # 准备格式化后用于绘图的特征数据
            X_shap_data_rounded = X_shap_data.round(1)
            force_plot_html = shap.force_plot(ev, sv, X_shap_data_rounded, show=False)
...
ensemble_components = {
    'selected_feature_list': sel_ft,      # 筛选后用于建模的特征
    'full_feature_list': feat_cols,       # 预处理时使用的全量特征
...
""")
print("=" * 120)
print("✅ 分析完成！POGD预测模型 v5.3".center(120))
print("=" * 120)

# ============================================================
# 【补充】可复现性辅助文件生成
# ============================================================
_step("生成requirements.txt环境配置文件...")
_req = """\
python==3.9.*
scikit-learn==1.2.2
xgboost==1.7.5
...
"""
with open(out('requirements.txt'), 'w', encoding='utf-8') as _f:
    _f.write(_req)
print(f"  ✅ 已生成: {out('requirements.txt')}")

_step("生成示例数据格式说明文件...")
_ex_cols = [id_col, ins_col, TARGET_COL] + feat_cols[:min(5, len(feat_cols))]
pd.DataFrame(columns=_ex_cols).to_csv(out('example_data_format.csv'), index=False, encoding='utf-8-sig')
print(f"  ✅ 已生成: {out('example_data_format.csv')}")
print("  ℹ️  请将您的CSV放到 data/ 子目录，并更新 DATA_PATH 常量。")